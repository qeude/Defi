...
- **List issues**: `gh issue list --limit 1000 --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'` with appropriate `--label` and `--state` filters.
...
- **Blocking**: GitHub's native issue dependencies are canonical. Where unavailable, use a `Blocked by: #<n>` line at the top of the child body.
